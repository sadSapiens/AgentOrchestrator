...
class BaseAgent(ABC):
    ...
    def __init__(self, name: str, role: str, system_prompt: str):
        ...
        logger.info(
            f"[bold green]✓[/bold green] Агент '{self.name}' ({self.role}) инициализирован"
        )
    ...
    async def execute(self, task: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            # Сохранить в историю
            self.history.append({"task": task, "context": context, "result": result})

            logger.agent_action(
                self.name, "Завершено", f"Результат: {len(result)} символов"
            )
    ...
    def save_result(
        self, content: str, directory: str = "results", filename: Optional[str] = None
    ) -> str:
        """
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {"name": self.name, "role": self.role, "history": self.history}

    def set_state(self, state: Dict[str, Any]):
        """Восстановить состояние агента"""
        if state.get("name") != self.name:
            logger.warning(
                f"Загрузка состояния: имя агента не совпадает ({state.get('name')} != {self.name})"
            )

        self.history = state.get("history", [])
        logger.info(
            f"Восстановлена история агента {self.name}: {len(self.history)} записей"
        )
